// Bice Workbench Service Worker v14
// Strategy: NO CACHING — always network-first, cache only as fallback
// Old caches (v8-v13) deleted on activate
const CACHE_NAME = 'bice-wb-v14';
const SW_VERSION = '14';

// Skip waiting: new SW takes control immediately
self.addEventListener('install', function(event) {
  console.log('[SW v14] Installing — skipWaiting enabled');
  self.skipWaiting();
});

// Activate: DELETE ALL OLD CACHES unconditionally
self.addEventListener('activate', function(event) {
  console.log('[SW v14] Activating — clearing all legacy caches');
  event.waitUntil(
    caches.keys().then(function(keys) {
      ...
      return self.clients.matchAll().then(function(clients) {
        clients.forEach(function(client) {
          client.postMessage({ type: 'sw-updated', version: 'v14' });
        });
      });
    })
  );
